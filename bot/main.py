...
from config import BOT_TOKEN, MINIAPP_URL, EMPLOYEE_CHAT_ID
    ...
    try:
        ...
        messages=[
            {"role": "system", "content": ('''
                ...
                Маршрут на карте: (ты строишь маршрут в Яндекс.картах и присылаешь ссылку на построенный маршрут)
                Заправки по маршруту с учетом среднего расхода топлива Х л/км для вашего мотоцикла Х:  (здесь ты указываешь информацию о заправках по маршруту)
                Информация о достопримечательностях: (здесь ты указываешь ссылки на информацию о достопримечательностях из дня маршрута)
                Отель: (ссылка на отель)
                Кафе: (ссылка на информацию о кафе)
...
async def process_route(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    response = await get_chatgpt_response(initial_prompt)
